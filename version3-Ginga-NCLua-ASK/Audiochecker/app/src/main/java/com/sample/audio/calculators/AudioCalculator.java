...
public class AudioCalculator {
    ...
    public int[] getAmplitudeLevels() {
        int i_major=0,i_minor=0;
        if (amplitudes == null) getAmplitudes();
        int major = 0;
        int minor = 0;
//        System.out.println("Amp length: "+amplitudes.length);
        for (int i =0; i < amplitudes.length; i++) {
//            System.out.println("Amp val: "+i+ " "+ amplitudes[i]);
            if (amplitudes[i] > major) {major = amplitudes[i]; i_major = i;}
            if (amplitudes[i] < minor) {minor = amplitudes[i]; i_minor = i;}
        }
        amplitude = Math.max(major, minor * (-1));
//        if (major == amplitude)
//            System.out.println("Amplitude max: "+ String.valueOf(i_major) +" - "+ major);
//        else if (minor == amplitude)
//            System.out.println("Amplitude max: "+ String.valueOf(i_minor)+" - "+ minor);
        return new int[] {major, minor};
    }

    public int getGivenAmplitude (int freq) {
        if (amplitudes == null) return 0;
        return Math.abs(amplitudes[freq]);
    }
...
//        System.out.println("Watch the amplitude: "+amplitude);
